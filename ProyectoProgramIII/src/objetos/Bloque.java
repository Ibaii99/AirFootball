...
import fisicas.PolarPoint;
import ventanas.VentanaGrafica;

/** Clase que permite crear y gestionar rect�ngulos
 * Realizada sin herencia (copiar-y-pegar desde Pelota)
 */
public class Bloque extends ObjetoFisico {
	 ...
	 * @param color	Color del bloque (p ej. 'a' = azul)
	 * @param bota	Informaci�n de si el bloque bota (true) o no (false)
	 */
	public Bloque(double x, double y, double ancho, double alto, char color, boolean bota) {
		super( x, y, color, bota );
		anchura = ancho;
		altura = alto;
	}
	
	/** Constructor de bloque con datos m�nimos. Crea un bloque azul que bota
	...
	public Bloque(double x, double y, double ancho, double alto) {
		this( x, y, ancho, alto, 'a', true );  // Reutilizar el constructor m�s espec�fico desde otro
	}
	...
	/** Calcula el �rea del bloque partiendo de su informaci�n
	 * @return	Area del bloque suponiendo un rect�ngulo perfecto
	 */
	@Override
	...
	public void dibuja( VentanaGrafica v ) {
		v.dibujaRect( x, y, anchura, altura, 1.5f, getAWTColor() );
	...
	/** Ajusta el bloque al suelo, si se ha "pasado" del suelo. Ajusta la velocidad a la que ten�a cuando lo toc�.
	...
	public void corrigeChoqueInferior( VentanaGrafica v, boolean dibujar ) {
		...
		if (velYInicial>0 && velocidadY>0) {
			double tiempoChoque = Fisica.calcTiempoHastaEspacio( velYInicial, antY, Fisica.GRAVEDAD, dondeToca);
			if (tiempoChoque > 0) {  // Si hay un error en el c�lculo no se aplica correcci�n
				...
				setVelocidadY( Fisica.calcVelocidad( velYInicial, tiempoMsgs, Fisica.GRAVEDAD )); // Corrige la velocidad
				if (dibujar) dibuja( v );
			}
		}
	}
	 ...
	 * @return	Devuelve null si no chocan, un vector con forma de punto indicando el �ngulo y amplitud del choque sobre el bloque en curso, 
	 * 			(0,0) si el bloque 2 incluye al bloque 1
	 */
	@Override
	public Point chocaConObjeto( ObjetoFisico objeto2 ) {
		if (objeto2 instanceof Bloque) {
				...
				if (bloque2.y > y+altura) {
					...
					if (bloque2.x+bloque2.anchura < x) {
						...
						if (bloque2.y+bloque2.altura < y) {
							return null;
						} else {  // Chocan. Veamos c�mo
							if (bloque2.x > x) {  // Bloque2 a la derecha del 1
								if (bloque2.x+bloque2.anchura<x+anchura) {  // El bloque2 est� "dentro" en horizontal
									if (bloque2.y > y) {  
										if (bloque2.y+bloque2.altura < y+altura) {  // El bloque2 est� "dentro" en vertical
									...
									if (bloque2.y > y) {  
										if (bloque2.y+bloque2.altura < y+altura) {  // El bloque2 est� "dentro" en vertical (a la derecha)
								...
								if (bloque2.y > y) {
									if (bloque2.y+bloque2.altura < y+altura) {  // El bloque2 est� "dentro" en vertical (a la izquierda)
								...
								if (bloque2.y > y) {  // Bloque2 abajo (toque abajo total)
									if (bloque2.y+bloque2.altura < y+altura) {  // El bloque2 est� "dentro" en vertical (toque horizontal total) {
			...
			if (cuadrante==1) {  // 1. Cuadrante izquierdo
				...
				pp.setModulo( pp.getModulo() - pelota.getRadio() );  // Vector de origen al punto m�s cercano de la esfera
			...
			} else {  // 5.- Nunca deber�a no estar en ning�n cuadrante
				return null;
			}
			return p;
		} else {
		...
		// Devuelve el cuadrante donde est� (1-izquierda, 2-derecha, 3-arriba, 4-abajo)
		private int cuadrante( double xPelota, double yPelota ) {
			...
			if (yPelota >= yDerechaDesde && yPelota <= yDerechaHasta) {
				if (cuad==1) {  //  Puede ser lo mismo izquierda que derecha... ver cu�l est� m�s cerca
			...
			if (xPelota >= xInfDesde && xPelota <= xInfHasta) {
				if (cuad==3) {  //  Puede ser lo mismo izquierda que derecha... ver cu�l est� m�s cerca
	 ...
	 * @return	true si el punto est� dentro del bloque, false en caso contrario
	 */
	@Override
